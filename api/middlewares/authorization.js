...
const dataEntryOperatorOnly = () => {
  return asyncHandler(async (req, res, next) => {
    if (
      ...
      res.status(403)
      throw new Error(
        'Only data entry operators are authorized to access this resource'
      )
    }
  })
}
...
const doctorAndDeoOnly = () => {
  return asyncHandler(async (req, res, next) => {
    console.log('doctor')
    if (req.user.role === 'dataEntryOperator' || req.user.role === 'doctor')
      next()
    else {
      res.status(403)
      throw new Error(
        'Only doctors and data entry operators authorized to access this resource'
      )
    }
  })
}
